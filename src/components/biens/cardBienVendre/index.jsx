...
const CardBienVendre = (props) => {
  // Sépare les biens non vendus et vendus
  const biensNonVendus = props.pages.filter(
    (page) => page.data.slices[0].primary.vendu !== true
  );
  const biensVendus = props.pages.filter(
    (page) => page.data.slices[0].primary.vendu === true
  );
  const toDisplay = biensNonVendus.length > 0 ? biensNonVendus : biensVendus;
  return (
    <Container className='md:flex md:flex-row md:flex-wrap md:w-full md:justify-center md:items-center '>
      {toDisplay.map((page, i) => (
        <Container
          key={i}
                ...
                <Container className='absolute bottom-1 left-1'>
                  {page.data.slices[0].primary.peb &&
                    page.data.slices[0].primary.peb !== "NO-PEB" && (
                      <MyImage
                        source={`/assets/peb-${page.data.slices[0].primary.peb
                          .split("-")[1]
                          ?.toLowerCase()}.png`}
                        h={20}
                        w={68}
